...
// NewPsqlRegistrationsRepository will create an object that represent the registration.Repository interface
func NewPsqlRegistrationsRepository(Conn *sql.DB, dbpg *pg.DB) registration.Repository {
	return &psqlRegistrationsRepository{Conn, dbpg}
}

func (regis *psqlRegistrationsRepository) CreateApplication(c echo.Context, app model.Applications,
	acc model.Account, pi model.PersonalInformation) error {
...
func (regis *psqlRegistrationsRepository) PostAddress(c echo.Context, acc model.Account) error {
	acc.UpdatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) PostSavingAccount(c echo.Context, acc model.Account) error {
	...
	_, err = stmt.Exec(acc.Application.SavingAccount, time.Now().Format(model.DateTimeFormatZone), time.Now(), acc.ApplicationID)
...
func (regis *psqlRegistrationsRepository) GetAccountByAppNumber(c echo.Context, acc *model.Account) error {
	newAcc := model.Account{}
	docs := []model.Document{}
	excludedStatus := []string{model.AppStatusInactive, model.AppStatusExpired}
...
func (regis *psqlRegistrationsRepository) GetAllRegData(c echo.Context, appNumber string) (model.PayloadBriRegister, error) {
	var plRegister model.PayloadBriRegister
	...
	if err != nil || (plRegister == model.PayloadBriRegister{}) {
...
func (regis *psqlRegistrationsRepository) UpdateAllRegistrationData(c echo.Context, acc model.Account) error {
	var nilFilters []string
...
func (regis *psqlRegistrationsRepository) PostOccupation(c echo.Context, acc model.Account) error {
	var nilFilters []string
	occ := acc.Occupation
...
func (regis *psqlRegistrationsRepository) GetZipcode(c echo.Context, addrData model.AddressData) (string, error) {
	var zipcode string
...
func (regis *psqlRegistrationsRepository) GetCityFromZipcode(c echo.Context, zipcode string) (model.AddressData, error) {
	addrData := model.AddressData{}
...
func (regis *psqlRegistrationsRepository) UpdateCardLimit(c echo.Context, acc model.Account, fnAfter func() error) error {
	var nilFilters []string
...
func (regis *psqlRegistrationsRepository) UpdateBrixkeyID(c echo.Context, acc model.Account) error {
	acc.UpdatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) GetAppStatus(c echo.Context, app model.Applications) (model.AppStatus, error) {
	...
	if err != nil || (appStatus == model.AppStatus{}) {
		logger.Make(c, nil).Debug(err)
...
func (regis *psqlRegistrationsRepository) UpdateAppStatusTimeout(c echo.Context, app model.Applications) error {
	appCheck := model.Applications{}
	app.UpdatedAt = model.NowDbpg()
	app.Status = model.AppStatusExpired

	err := regis.DBpg.Model(&appCheck).
		Where("application_number = ? and status = ?", app.ApplicationNumber, model.AppStatusOngoing).Limit(1).Select()
	...
	_, err = regis.DBpg.Model(&app).Set("status = ?status, updated_at = ?updated_at").
		Where("application_number = ? and status = ?", app.ApplicationNumber, model.AppStatusOngoing).
		Update()
...
func (regis *psqlRegistrationsRepository) ForceUpdateAppStatusTimeout() error {
	...
	_, err := regis.DBpg.Model(app).Exec(`UPDATE applications SET status = ?, updated_at = ?
		where expired_at <= ? and status = ?`, model.AppStatusExpired, model.NowDbpg(), model.NowDbpg(),
		model.AppStatusOngoing)
...
func (regis *psqlRegistrationsRepository) UpdateAppStatus(c echo.Context, app model.Applications) error {
	app.UpdatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) ResetAppStatusToCardProcessed(appsId int64) error {
	...
	_, err := regis.DBpg.Model(app).Exec(`UPDATE applications SET status = ?, card_processed_date = ?, card_sent_date = null, card_send_date = null, 
		rejected_date = null where id = ?`, model.AppStatusCardProcessed, model.NowDbpg(), appsId)
...
func (regis *psqlRegistrationsRepository) UpdateAppDocID(c echo.Context, app model.Applications) error {
	app.UpdatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) UpdateApplication(c echo.Context, app model.Applications, col []string) error {
	app.UpdatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) UpsertAppDocument(c echo.Context, doc model.Document) error {
	var err error

	if doc.ID == 0 {
		err = regis.insertAppDocument(c, doc)
	} else {
		doc.UpdatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) GetCoreServiceStatus(c echo.Context) error {
	...
	if err != nil || (param == model.Parameter{}) {
		logger.Make(nil, nil).Debug(err)
...
func (regis *psqlRegistrationsRepository) UpdateCoreOpen(c echo.Context, acc *model.Account) error {
	app := model.Applications{
		CoreOpen:  true,
		UpdatedAt: model.NowDbpg(),
	}
...
func (regis *psqlRegistrationsRepository) insertAppDocument(c echo.Context, doc model.Document) error {
	doc.CreatedAt = model.NowDbpg()
...
func (regis *psqlRegistrationsRepository) GetDocumentByApplicationId(appId int64, docType string) ([]model.Document, error) {
	var listDocument []model.Document
...
func (regis *psqlRegistrationsRepository) DeactiveAccount(c echo.Context, acc model.Account) error {
	var nilFilters []string
	acc.Status = model.AccStatusInactive
...
func (regis *psqlRegistrationsRepository) GetAppOngoing() ([]model.Account, error) {
	accs := []model.Account{}

	err := regis.DBpg.Model(&accs).Relation("Application").Relation("PersonalInformation").
		Where("Application.expired_at > ? and Application.status = ?", model.NowDbpg(),
			model.AppStatusOngoing).Select()
...
func (regis *psqlRegistrationsRepository) ForceDeliverAccount(c echo.Context, acc model.Account) error {
	...
	_, err = stmt.Exec(model.AppStatusForceDeliver, time.Now(), acc.Application.ID)
...
func (regis *psqlRegistrationsRepository) GetAppByCIF(cif string) (model.Applications, error) {
	acc := model.Account{}
